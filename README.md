...
| [PR-Agent](https://github.com/Codium-ai/pr-agent) | Automates PR review and validation using Gemini models. | Configured via GitHub Actions (`.github/workflows/pr-agent.yml`) and `.pr_agent.toml`. Requires `OPENAI_KEY` or `GEMINI_API_KEY` repository secret. |
